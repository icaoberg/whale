...
function whaleosc1( epsilon, option1, k, option2, px0, x0, y0 )
...
if nargin == 0
    ...
    if option == 3
         betax = input( 'Please input a value for betax :' );1
...
for t = 12:c %Counter for number of generations
    x(t) = x_foscsine1( epsilon, betax, mux, muj, muy, x(t-1), y(t-1), px(t-1), px(t-k), k, t ); 
    y(t) = y_f( betay, mux, muy, y(t-1), px(t-k) );
    px(t) = px_foscsine1( epsilon, muj, mux, muy, x(t-1), y(t-1), t );
...
title(['Population of ... with perturbation on the paired female and single female classes of  ' num2str(epsilon) '.'])
...
xlabel('Time')
ylabel('Single Females')
title(['Population of ... with perturbation on the paired females and single female classes of  ' num2str(epsilon) '.'])
